def add_contact(tokens: list, contacts: dict) -> str:
    ...
    Raises:
        ValueError: If the number of tokens is less than 2 or if the name already exists in the contacts dictionary.
    """
    if len(tokens) < 2:
        raise ValueError("Add Contact: Not enough arguments. Input: add <name> <phone>")
...
def change_contact(tokens: list, contacts: dict) -> str:
    ...
    Returns:
        str: A string indicating the result of the operation.
    Raises:
        ValueError: If the number of tokens is less than 2 or if the name does not exist in the contacts dictionary.
    """
    if len(tokens) < 2:
        raise ValueError("Change Contact: Not enough arguments. Input: change <name> <phone>")
...
def show_phone(tokens: list, contacts: dict) -> str:
    ...
    Raises:
        ValueError: If the number of tokens is less than 1, indicating that the name of the contact was not provided.
    """
    if len(tokens) < 1:
        raise ValueError("Show phone: Not enough arguments. Input: phone <name>")
